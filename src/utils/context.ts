...
function walkDirectory(root: string, maxDepth: number, maxEntries: number): {
  ...
  function walk(current: string, depth: number): TreeNode | null {
    ...
    const isDir = fs.existsSync(current) && fs.statSync(current).isDirectory();
    ...
    if (isDir) {
      ...
      entries.sort((a, b) => {
        const aDir = fs.existsSync(a) && fs.statSync(a).isDirectory();
        const bDir = fs.existsSync(b) && fs.statSync(b).isDirectory();
...
function buildMarkdown(ctx: ProjectContextJson): string {
  ...
  const langLines = ctx.summary.languages.slice(0, 12).map(l => `- .${l.extension}: ${l.files}`);
